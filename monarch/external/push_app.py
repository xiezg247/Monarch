    ...
    def deal_response(self, response):
        try:
            if response and response.status_code == 200:
                ...
                logger.info('远端服务器返回错误:{} {} {}'.format(response.status_code, self.url, str(response.content)))
                return BIZ_REMOTE_SERVICE_NOT_VALID, response.json()
        except Exception as err:
            logger.info('远端服务器返回数据无法解析:{} {}'.format(str(response.content), err))
            return BIZ_REMOTE_DATA_NOT_VALID, {}

    def send_company_data(self, data):
        try:
            response = self.s.post(url=self.url,
                                   json=data,
                                   timeout=REQUESTS_TIMEOUT)
            logger.info('url: {}, data: {}, response: {}'.format(self.url, data, response.content))
